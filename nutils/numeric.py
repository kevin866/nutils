...
def sanitize_einsum_subscripts(subscripts, *shapes):
    ...
    explicit = '->' in subscripts
    in_, out = subscripts.split('->', 1) if explicit \
        else (subscripts, ''.join(sorted(c for c in einsum_symbols_set.intersection(subscripts) if subscripts.count(c) == 1)))
    in_ = in_.split(',')
    ...
    if any(all(c not in s for s in in_) for c in out):
        raise ValueError('output contains subscripts that do not occur in input')
    return (*in_, out)
