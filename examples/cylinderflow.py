...
from calling import PassThrough, Cached
import itertools
import numpy
import treelog
...
def main(nelems: int = 99,
         ...
         extdiam: float = 12.,
         endtime: float = 30.,
         calling: Union[PassThrough, Cached] = PassThrough()):
    ...
    topo, ns = calling[construct_domain_namespace](nelems, extdiam, reynolds, degree, timestep, uwall)
    cons = calling[project_constraints](topo, ns, degree)
    res = calling[define_problem](topo, ns, degree)

    bbox = 4 * numpy.array([[-1/2, 16/9 - 1/2], [-1/2, 1/2]])
    bbtopo = calling[prepare_plotting_topology](topo, ns, bbox)

    args = calling[construct_initial_condition](topo, ns, degree, cons)
    xgrd = None
    ...
    for time in treelog.iter.fraction('timestep', numpy.arange(0, endtime, timestep)) if endtime < float('inf') \
        ...
        args = calling[progress_timestep](res, cons, args)
        xgrd = calling[plot_solution](bbtopo, ns, args, time, bbox, xgrd)

    return args, numpy.sqrt(calling.get(topo).integral('∇_k(u_k)^2 dV' @ ns, degree=2))
